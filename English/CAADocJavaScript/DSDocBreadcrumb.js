...
if (IE) {
    window.onresize =updateBreadcrumbFrame;
//} else if (NS7) {
} else if (Firefox || Chrome) {
    if (window.frameElement.name=="Breadcrumb") {
        window.onresize=updateBreadcrumbFrame;
...
//	DebugMessage("Init Breadcrumb");
	...
	if(mainWin.document && mainWin.document.location)
		...
		//setTimeout( function(){ updateBreadcrumb(mainWin) },2000); // V2E : IR-096045V6R2012x
}

/*
* onresize Updates the breadcrumb frame 
*/
function updateBreadcrumbFrame(){
var frame = parent.parent.frames["RightFS"].document.getElementById("BreadCrumbFrame");
  ...
  if(breadcrumbStrLength>=linelength)	{
...
//alert('After  breadCrumbLength = '+breadCrumbLength);
    	...
    	// V2E : For each parent " | " this String is to be considered
    	var breadCrumbLengthUpdated = breadCrumbLength + ((i-1)*3); 
		...
		breadcrumbStrLength = breadCrumbLengthUpdated;
		// increase the size of frame if required
		if(breadCrumbLengthUpdated>linelength)
		{
		parent.parent.frames["RightFS"].document.getElementById("BreadCrumbFrameset").rows="40,*";
		}
		// test code
