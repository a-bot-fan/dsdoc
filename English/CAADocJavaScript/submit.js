...
function CAAlinkJava(iPackage, iInterface, iMethod)
{
  ...
  if (!iPackage)
  {
    url = "../docs/api/_index/jPackageView.html";
  }
  else
  {
    ...
    else
    {
      if (!iInterface)
      {
        url = "../docs/api/_index/pkg/" + iPackage + ".html";
      }
      else
      {
        if (!iMethod)
...
{
	document.writeln('<p class="copyright"><strong>3D</strong>EXPERIENCE R2016x &#169; 2015 Dassault Syst&#232;mes. All rights reserved.<br />');
...
function caaGetJSSampleURL(iSamplePath)
{
  return '../../devrun/webapps/'+iSamplePath;
}


function caaLinkSample(iSamplePath)
{
  return CAAlinkOutOfPreqs(caaGetJSSampleURL(iSamplePath));
}
